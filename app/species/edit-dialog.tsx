...
import { z } from "zod";
import { type Species } from "./learn-more-dialog";
...
export default function EditSpecies({ userId, species }: { userId: string, species: Species }) {
  const router = useRouter();
  const [open, setOpen] = useState<boolean>(false);

  const form = useForm<FormData>({
    resolver: zodResolver(speciesSchema),
    defaultValues: {...species} as Partial<FormData>,
    mode: "onChange",
  });
  ...
  const onSubmit = async (input: FormData) => {
    ...
    const res = await supabase.from("species").update(      {
        author: userId,
        common_name: input.common_name,
        description: input.description,
        kingdom: input.kingdom,
        scientific_name: input.scientific_name,
        total_population: input.total_population ?? 0,
        image: input.image,
      },
    ).eq('id', species.id).select();
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-screen overflow-y-auto sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Edit a new species here. Click &quot;Save Species&quot; below when you&apos;re done.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
